...
color and height of the hexagons relate to the number of stations present in
each hexagon.
...
* Scroll in our out to control the zoom.
...
In this section, the points of the point grid are coloured by the median travel
time taken by bike from every charging station within a 30 minute ride. The
routing method takes account of the public road network, including
...
Points in the grid that were not reachable within 30 minutes appear red. Points
with a lower median travel time are coloured white. The pink zone within the
red points represents the effective extent that can be reached by the average
cyclist within a 30 minute cycle from any of the station locations. 
...
Note that the point grid is naive to the locations of places that people may
wish to cycle to. The current approach could be easily adapted to use locations
of overnight population, commercial areas or any other geolocatable feature of
interest.

Note that some of the locations within the point grid were adjusted by a
...
time. For areas in truly inaccessible areas such as waterways, landuse spatial
...
more note that due to the naive locations of the point grid, these locations
...
interest. Alternative methods of constrained network optimisation can also
be explored in order to maximise coverage of the cycle network.
